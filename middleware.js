...
export function middleware(request) {
    ...
    const pathTraversalRegex = /(\.\.[\/\\])|(%2e%2e[%2f%5c])|(%252e%252e%25[2f5][cf])/i;
    ...
    const checks = {
        ...
        suspiciousPattern: suspiciousPatterns.some((pattern) => decodedPath.toLowerCase().includes(pattern.toLowerCase())),
    ...
    if (isBlocked) {
        ...
        const response = NextResponse.redirect(new URL('/error?code=403&reason=security', request.url));
...
function setSecurityHeaders(response, url, request, isBlocked = false) {
    ...
    const isHTTPS = url.protocol === 'https:' || request.headers.get('x-forwarded-proto') === 'https' || request.headers.get('x-forwarded-ssl') === 'on';
